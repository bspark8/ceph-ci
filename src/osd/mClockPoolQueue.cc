...
namespace ceph {
    ...
    const mClockPoolQueue::InnerClient& client)
  {
	...
	return client_info_mgr.get_pool_default();
      ...
      auto client_it = client_info_mgr.cli_info_map.find(client.first);
      if (client_info_mgr.cli_info_map.end() != client_it) {
	*client_it->second = dmc::ClientInfo(pp->get_qos_res(),
					     pp->get_qos_wgt(),
					     pp->get_qos_lim());
